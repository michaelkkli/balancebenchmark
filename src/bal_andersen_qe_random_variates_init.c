...
{
  size_t m = param->num_path_blocks * param->path_block_size * param->num_steps;
  uint64_t * buf;

  _bal_andersen_qe_random_variates rv = (typeof(rv))malloc(sizeof (_bal_andersen_qe_random_variates_internal)); if (!rv) return;

  rv->rand = (typeof(rv->rand))malloc(sizeof(double) * m * 3);
  if (!rv->rand)
    {
      free(rv);
      rv = 0;

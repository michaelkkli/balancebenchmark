...
void
_bal_andersen_qe_minstore (double *S, _bal_andersen_qe_random_variates rv,
			   _bal_andersen_qe_param param)
{
  ...
  s2A =
    (param->epsilon * param->epsilon * expmkd / param->kappa) * one_m_expmkd;
  s2B =
    (param->theta * param->epsilon * param->epsilon / 2. / param->kappa) *
    one_m_expmkd * one_m_expmkd;

  K0 =
    -(param->rho * param->kappa * param->theta / param->epsilon) *
    param->delta_t;
  ...
  for (b = 0; b < param->num_path_blocks; b++)
      ...
      for (i = 0; i < param->path_block_size; i++)
	{
	  ...
	  for (t = 0; t < param->num_steps; t++)
		...
		+ sqrt (K3 * tprev->V +
			K4 * tnow->V) * rv->Z[Sindex * param->num_steps + t];
